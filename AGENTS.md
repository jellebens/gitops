...
- **A manual sync from the UI can drop `ServerSideApply` and then poison auto-sync.** The kyverno CRDs (`clusterpolicies.kyverno.io`, `policies.kyverno.io`) are >256 KiB, so a client-side apply fails with `metadata.annotations: Too long: may not be more than 262144 bytes` (the `last-applied-configuration` annotation). That is what happens when the UI sync dialog is submitted with its own option set (e.g. `RespectIgnoreDifferences` ticked but not `ServerSideApply`), and Argo then does **not** auto-retry the same revision, so the app sits on a `SyncError` condition. Fix: `argocd app sync <app> --core` (the CLI uses the app's own `syncOptions`); never untick ServerSideApply in the UI for apps that carry it.
- **Argo's local "structured-merge diff" (the default diff for every app with `ServerSideApply=true`) drops CRD-schema defaults inside atomic lists → perpetual OutOfSync + a self-heal loop of no-op applies every 5 min.** Seen on the CNPG `Cluster` (`spec.managed.roles[].connectionLimit: -1`) and on kyverno `ClusterPolicy` (`spec.rules[].skipBackgroundRequests`, `rules[].validate.allowExistingViolations`): the live object carries the default, the predicted state does not, `argocd app diff` shows nothing, and every "successful" sync changes nothing (the `managedFields` timestamp never moves). Upstream calls this diff discontinued for exactly this reason. Fix of record: opt the affected Application into the real **server-side diff** (a dry-run apply, so defaults are present) with `argocd.argoproj.io/compare-options: ServerSideDiff=true` on the child Application (ceres, kyverno-config, and kyverno itself — its `policies.kyverno.io` CRDs showed the same phantom diff). Verify a candidate with `argocd app diff <app> --core --server-side-generate` (empty = it will converge). Not enabled cluster-wide on purpose: the same preview showed chaos-mesh's generated cert Secrets would newly diff under it.
- **Child Applications OutOfSync in `bootstrap` only on `metadata.finalizers`** (`pre-delete-finalizer.argocd.argoproj.io`, `.../cleanup`): Argo CD 3.x adds/removes those itself on every reconcile for apps whose chart has Helm delete hooks (longhorn, kyverno). Do not copy them into git; `platform/argocd-config` `cm.yaml` ships a system-level `ignoreDifferences` for `argoproj.io/Application` that hides them.
...
- **The CNI must never depend on DNS that lives behind the CNI (2026-09-10 outage, ~2 days).** The lab's primary DNS is `coredns-lab` on the Cilium LB VIP `.180`; the nodes' fallback resolver is the DS918 slave `.144`, which answers an *authoritative NXDOMAIN* for bare `.local` names (glibc then never tries the router). Cilium was configured with `k8sServiceHost: k3s-master01.local`, so once the agents restarted they crash-looped on `lookup k3s-master01.local ... no such host` (the `config` container, `kubectl -n kube-system logs <cilium-pod> -c config`), with no way to bring the `.180` VIP back — a deadlock. **Symptoms look deceptively healthy:** nodes `Ready`, Argo `Synced/Healthy`, existing pods still running — but `ds/cilium` is 0/N Ready, every new pod is `FailedCreatePodSandBox … cilium.sock: no such file`, Longhorn's CSI plugin drops off (`driver.longhorn.io not found in the list of registered CSI drivers`), and every Longhorn-backed workload fails to mount on restart. **Always check `kubectl get ds -A` and non-Running pods, never trust node/app status alone.** Fix of record: homelab `dd52709` pins `k3s_api` (→ `k8sServiceHost`) to the control-plane IP `192.168.50.151` (node IPs are in the apiserver cert SANs); applied with `ansible-navigator run playbooks/deploy_k3s.yml --tags cilium -i inventories/shared -i inventories/lab/k3s.yml` (the `cilium` tag only — the untagged full play's network role **reboots all nodes in parallel**; and pass `-i` explicitly, `ansible.cfg`'s inventory list does not parse inside the EE). Standing caveat: nodes still cannot resolve bare `.local` names (`.144` is first in `roles/network/tasks/wired.yml`) — use `lab.local` names for anything node-side. While LAN DNS is down, WSL needs `/etc/hosts` entries for `k3s-*.local` and `kubectl --server https://192.168.50.151:6443 --tls-server-name k3s-master01.local` works; LB VIPs (`.180`, `.200`) never answer ICMP — test with `nslookup`/`dig`, not `ping`.
- **Gateway API CRDs are a homelab bootstrap concern, and they must match the Cilium version (2026-09-15, ceres-firmware route).** Cilium is installed by homelab `roles/cilium` (Helm, pinned `cilium_chart_version`), and the same role applies the Gateway API CRD bundle at `cilium_gateway_api_version` BEFORE the chart, because the operator checks for them at startup: with a missing or too-old bundle its Gateway API controller does not start at all (`kubectl -n kube-system logs -l io.cilium/app=operator | grep "Required GatewayAPI resources"`). **Symptoms look deceptively healthy:** routes programmed earlier keep serving, but every HTTPRoute created afterwards has an EMPTY `.status` (no `Accepted` condition), Envoy answers 404 for its hostname on every path, and the owning Argo app sits at `Progressing`. Diagnose with `kubectl get httproute -A -o json | jq '.items[] | select(.status.parents == null) | .metadata.name'`. Nothing in this repo owns those CRDs (Argo comes up after the CNI), so never hand-apply a bundle: bump `cilium_chart_version` and `cilium_gateway_api_version` together in homelab (each Cilium minor documents the release it needs) and rerun `playbooks/deploy_k3s.yml --tags cilium`. Everything else Gateway-shaped IS gitops: the Gateway, its listeners, all HTTPRoutes and ReferenceGrants live in `platform/gateway-config` (+ `.config/lab/gateway.yaml`) or the landing zones, and the GatewayClass `cilium` is created by the Cilium chart itself.
